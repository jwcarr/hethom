...
To evaluate this argument statistically, we regressed cost on generation in a Bayesian, hierarchical, polynomial regression model. 
    \begin{align*}
       c_i & \sim \mathrm{Normal}(\mu_i, \sigma) \\
     \mu_i & =    \alpha_{\mathrm{CHAIN}[i]} + \beta_{\mathrm{CHAIN}[i]} g_i + \gamma_{\mathrm{CHAIN}[i]} g_i^2 + \delta_{\mathrm{CHAIN}[i]} g_i^3 \\
...
[N.B., I haven't completed this bit yet] To evaluate statistically the effect of generation (modulated by epoch) on cost, we fit the following Bayesian, hierarchical regression model
    ...
    \begin{align*}
       c_i & \sim \mathrm{Normal}(\mu_i, \sigma) \\
     \mu_i & =    \alpha_{\mathrm{CHAIN}[i]} + \beta_{\mathrm{CHAIN}[i]} g_i \\
    \sigma & \sim \mathrm{Exponential}(1)
    \end{align*}
