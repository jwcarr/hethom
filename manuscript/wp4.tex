...
Even the most orthographically transparent of languages have at least some techniques for disambiguating homophones in writing. Italian, for example, uses the grave accent to distinguish common homophonous words: <la> (\textit{the}) vs.\@ <là> (\textit{there}). Many other languages employ various diacritics for the same purpose, such as the acute accent in Danish---<en> (\textit{a}) vs.\@ <én> (\textit{one})---or the circumflex in Serbo-Croatian---<da> (\textit{yes}) vs.\@ <dâ> (\textit{gives}).
